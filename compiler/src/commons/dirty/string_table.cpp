...
StringIndex StringTable::intern(const std::string& str) {
    ...
    StringIndex index{static_cast<uint32_t>(strings_.size())};
...
const std::string& StringTable::get_string(StringIndex index) const {
    if (index.value >= strings_.size()) {
        throw std::out_of_range("StringTable: Invalid string index " + std::to_string(index.value) + 
                               ", table size is " + std::to_string(strings_.size()));
    }
    return strings_[index.value];
}
