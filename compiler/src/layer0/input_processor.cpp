...
std::map<std::string, std::stringstream> InputProcessor::process_input_files(
    ...
    LOG_DEBUG("Processing {} input files", params.input_files.size());
    
    // Process each input file
    for (const auto& file_path : params.input_files) {
        LOG_DEBUG("Processing file: {}", file_path.string());
        
        // Validate file
        if (!is_file_readable(file_path)) {
            // TODO: Implement proper error handling with Result<T> type system
            LOG_ERROR("File not readable: {}", file_path.string());
            return {}; // Return empty map to signal error
        }
        
        if (!has_valid_extension(file_path)) {
            // TODO: Implement proper error handling with Result<T> type system
            LOG_ERROR("Invalid file extension: {} (expected .cp or .cprime)", file_path.string());
            return {}; // Return empty map to signal error
        }
        ...
        LOG_DEBUG("Generated stream ID '{}' for file {}", stream_id, file_path.string());
        input_streams[stream_id] = std::move(file_stream);
    }
    
    LOG_INFO("Successfully processed {} input files", input_streams.size());
...
std::stringstream InputProcessor::read_file(const std::filesystem::path& file_path) {
    ...
    if (file.bad()) {
        // TODO: Implement proper error handling with Result<T> type system
        LOG_ERROR("Error reading file: {}", file_path.string());
        stream.setstate(std::ios::failbit);
    } else {
        LOG_DEBUG("Successfully read {} characters from {}", 
